	...
	for(var i = 0; i < this.buffer.getBufferHeight(); i++) {
		...
		if(current_line) {
			for(var j = 0; j < current_line.str.length; j++) {
				...
				var current_attr = current_line.attr[parseInt(j,10).toString()];
				if (current_attr) {
				...
				if ( j === current_line.str.length - 1) {
					var end_attr = current_line.attr[current_line.str.length.toString()];
					if (end_attr) {
						ansi_line.push(ansi_sgr_code(end_attr));
					}
				}
			}
		}
